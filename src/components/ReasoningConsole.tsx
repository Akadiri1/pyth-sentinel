...
export default function ReasoningConsole({ logs }: ReasoningConsoleProps) {
  ...
  return (
    <div className="glass-card flex flex-col h-[360px] sm:h-[420px] scanline-overlay">
      {/* Header */}
      <div className="flex items-center justify-between px-3 sm:px-4 py-2 sm:py-2.5 border-b border-pyth-border">
        <div className="flex items-center gap-2">
          <Terminal className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-pyth-purple" />
          <h2 className="font-mono text-[10px] sm:text-xs font-semibold text-pyth-text-dim tracking-wider">
            REASONING ENGINE
          </h2>
        </div>
        <div className="flex items-center gap-2 sm:gap-3">
          <span className="font-mono text-[8px] sm:text-[9px] text-pyth-green/70 hidden sm:inline">
            LATENCY: {(0.3 + Math.random() * 0.6).toFixed(0)}ms
          </span>
          <div className="w-1.5 h-1.5 rounded-full bg-pyth-green animate-pulse" />
          <span className="font-mono text-[9px] sm:text-[10px] text-pyth-text-muted">
            {logs.length} events
          </span>
        </div>
      </div>
      ...
      <div
        ref={scrollRef}
        className="flex-1 overflow-y-auto custom-scrollbar p-2 sm:p-3 space-y-1"
      >
            ...
            return (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  } ${index > 2 ? 'line-clamp-2' : ''}`}>
                    {log.message.length > 140 ? log.message.slice(0, 140) + '…' : log.message}
                  </p>
                </div>
              </motion.div>
            );
      ...
      <div className="px-3 sm:px-4 py-1.5 sm:py-2 border-t border-pyth-border flex items-center justify-between">
        <div className="flex items-center gap-2">
          <Crosshair className="w-3 h-3 text-pyth-purple animate-spin" style={{ animationDuration: '3s' }} />
          <span className="font-mono text-[9px] sm:text-[10px] text-pyth-text-muted">
            <span className="hidden sm:inline">Chain-of-Thought reasoning active</span>
            <span className="sm:hidden">CoT active</span>
          </span>
        </div>
        <div className="flex items-center gap-1.5">
          <div className="w-1.5 h-1.5 rounded-full bg-pyth-green sentinel-blink" />
          <span className="font-mono text-[8px] sm:text-[9px] text-pyth-green/80 tracking-widest sentinel-blink">
            <span className="hidden sm:inline">SENTINEL_V1.0_READY</span>
